...
def plot_eigenspectrum(model, digit, eigenvectors=3, eigenvalues=20, ignore_pos=[], ignore_neg=[]):
    ...
    fig.update_layout(
        title=f"ΔB[{digit}] Eigenspectrum",
        title_y=0.98,  # Move title down slightly
        height=400,     # Increase height to fit title
        margin=dict(t=60, b=40, l=40, r=40),  # Add top margin for title
        showlegend=False
    )
...
def plot_delta_eigenspectrum(model1, model2, digit, eigenvectors=3, eigenvalues=20, ignore_pos=[], ignore_neg=[]):
    """Plot the eigenspectrum of the delta interaction matrix between two models."""
    colors = px.colors.qualitative.Plotly
    fig = make_subplots(rows=2, cols=1 + eigenvectors)
    
    # Get interaction matrices and compute delta
    B1 = get_interaction_matrix(model1)
    B2 = get_interaction_matrix(model2)
    delta_B = (B2 - B1)[digit].cpu()
    
    # Eigendecomposition of delta
    vals, vecs = torch.linalg.eigh(delta_B)
    
    # Project eigenvectors to input space (using model2's embedding)
    vecs_input = einsum(vecs, model2.w_e.cpu(), "h c, h i -> c i")
    ...
    # Top row: eigenvectors for positive eigenvalues
    for i, idx in enumerate(positive):
        fig.add_trace(go.Heatmap(z=vecs_input[idx].view(28, 28).flip(0), colorscale="RdBu", zmid=0, showscale=False), row=1, col=i+2)

    # Bottom row: eigenvectors for negative eigenvalues
    for i, idx in enumerate(negative):
        fig.add_trace(go.Heatmap(z=vecs_input[idx].view(28, 28).flip(0), colorscale="RdBu", zmid=0, showscale=False), row=2, col=i+2)
